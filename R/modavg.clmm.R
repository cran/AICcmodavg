modavg.clmm <-
  function(cand.set, parm, modnames, conf.level = 0.95, second.ord = TRUE, nobs = NULL, exclude = NULL,
    ...
    if(!identical(check.class, "clmm"))  {stop("\nThis function is only appropriate with the \'clmm\' class\n")}
...
######################################################################################################
  ...
  #####################################################
    ...
    new_table <- aictab.clmm(cand.set=new.cand.set, modnames=new.mod.name, sort=FALSE,
