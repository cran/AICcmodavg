...
if (any(check_ML!="ML")) warning(paste("Model selection for fixed effects is only appropriate with method=ML:", "\n",
...
if(identical(check.method, c("ML", "REML"))) {stop("You should not have models fit with REML and ML in the same candidate model set")}
...
if(second.ord==FALSE) {colnames(Results)<-c("Modnames", "K", "AIC", "Delta AIC", "ModelLik", "AICWt", "Cum.Wt")}  #rename correctly to AIC
