modavg.shrink.coxph <- function(cand.set, parm, modnames, conf.level = 0.95, second.ord = TRUE, nobs = NULL,
                                uncond.se = "revised"){
  ...
  if(identical(check.class, "coxph") || identical(check.class, c("coxph.null", "coxph"))) {
    ...
    ##compute table
    new_table <- aictab.coxph(cand.set = cand.set, modnames = modnames, sort = FALSE,
