...
aic.table <- aictab(cand.set=cand.set, modnames=modnames, sort=TRUE, c.hat=c.hat, second.ord=second.ord, nobs=nobs)
...
if (method=="ratio") {
  ...
  confset.tab <- list("method" = method, "cutoff" = cutoff, "delta" = delta, "table" = aic.table[which(ratios> cutoff),])
}
