aictab.rlm <-
  function(cand.set, modnames, sort = TRUE, second.ord = TRUE, nobs = NULL){  #specify whether table should be sorted or not by delta AICc
    ...
    Results$K <- unlist(lapply(cand.set, AICc.rlm, return.K = TRUE, second.ord = second.ord, nobs = nobs))     #extract number of parameters
    Results$AICc <- unlist(lapply(cand.set, AICc.rlm, return.K = FALSE, second.ord = second.ord, nobs = nobs))  #extract AICc                                      #
