...
#check if all are identical
  ...
  if(identical(check.class, "lm") || identical(check.class, c("glm", "lm"))) {results <- aictab.glm(cand.set=cand.set, modnames=modnames, sort=sort, c.hat=c.hat, second.ord=second.ord, nobs=nobs); known[1] <- 1}   #determine if lm or glm

  if(identical(check.class, "lme"))  {results <- aictab.lme(cand.set=cand.set, modnames=modnames, sort=sort, second.ord=second.ord, nobs=nobs); known[2] <- 1}      #determine if lme
