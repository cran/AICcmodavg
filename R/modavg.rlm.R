modavg.rlm <-
  function(cand.set, parm, modnames, conf.level = 0.95, second.ord = TRUE, nobs = NULL, exclude = NULL,
    ...
    if(!identical(check.class, c("rlm", "lm")))  {stop("\nThis function is only appropriate with the \'rlm\' class\n")}
...
######################################################################################################
  ...
  #####################################################
    ...
    new_table <- aictab.rlm(cand.set=new.cand.set, modnames=new.mod.name, sort=FALSE,
