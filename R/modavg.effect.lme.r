...
function(cand.set, modnames, newdata, conf.level = 0.95, second.ord = TRUE, nobs = NULL, uncond.se = "revised"){
  ...
  if(identical(check.class, "lme") )  {
    ...
    fit <- matrix(data = unlist(lapply(X = cand.set, FUN = function(i)predictSE.lme(i, se.fit = TRUE, newdata = newdata)$fit)),
                  nrow = nmods, ncol = 2, byrow = TRUE)
    
    ##compute SE's on fitted values
    SE <- matrix(data = unlist(lapply(X = cand.set, FUN = function(i)predictSE.lme(i, se.fit = TRUE, newdata = newdata)$se.fit)),
                 nrow = nmods, ncol = 2, byrow = TRUE)
