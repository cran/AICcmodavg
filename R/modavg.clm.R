modavg.clm <-
  function(cand.set, parm, modnames, conf.level = 0.95, second.ord = TRUE, nobs = NULL, exclude = NULL,
    ...
    if(!identical(check.class, c("sclm", "clm")))  {stop("\nThis function is only appropriate with the \'clm\' class\n")}
...
######################################################################################################
  ...
  #####################################################
    ...
    new_table <- aictab.clm(cand.set=new.cand.set, modnames=new.mod.name, sort=FALSE,
