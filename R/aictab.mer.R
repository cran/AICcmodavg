...
#check if models were fit with same method (REML or ML)
    ...
    Results$K<-unlist(lapply(cand.set, AICc.mer, return.K = TRUE, second.ord = second.ord, nobs = nobs))     #extract number of parameters
    Results$AICc<-unlist(lapply(cand.set, AICc.mer, return.K = FALSE, second.ord = second.ord, nobs = nobs))  #extract AICc                                      #
...
#check if models were fit with same method (REML or ML)
    ...
    Results<-data.frame(Modnames = modnames)                    #assign model names to first column
    Results$K<-unlist(lapply(cand.set, AICc.merMod, return.K = TRUE, second.ord = second.ord, nobs = nobs))     #extract number of parameters
    Results$AICc<-unlist(lapply(cand.set, AICc.merMod, return.K = FALSE, second.ord = second.ord, nobs = nobs))  #extract AICc                                      #
    Results$Delta_AICc<-Results$AICc-min(Results$AICc)            #compute delta AICc
    Results$ModelLik<-exp(-0.5*Results$Delta_AICc)                #compute model likelihood required to compute Akaike weights
    Results$AICcWt<-Results$ModelLik/sum(Results$ModelLik)        #compute Akaike weights
