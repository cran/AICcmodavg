...
#check if models were fit with same method (REML or ML)
    ...
    Results$K <- unlist(lapply(cand.set, AICc.nlme, return.K = TRUE, second.ord = second.ord, nobs = nobs))     #extract number of parameters
    Results$AICc <- unlist(lapply(cand.set, AICc.nlme, return.K = FALSE, second.ord = second.ord, nobs = nobs))  #extract AICc                                      #
