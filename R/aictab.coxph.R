aictab.coxph <- function(cand.set, modnames, sort=TRUE, second.ord=TRUE, nobs=NULL){  #specify whether table should be sorted or not by delta AICc
  ...
  Results$K <- unlist(lapply(cand.set, AICc.coxph, return.K = TRUE, second.ord = second.ord, nobs = nobs))     #extract number of parameters
  Results$AICc <- unlist(lapply(cand.set, AICc.coxph, return.K = FALSE, second.ord = second.ord, nobs = nobs))  #extract AICc                    
  ...
  Results$LL <- unlist(lapply(X=cand.set, FUN=function(i) extract.LL.coxph(i)[1]))      
