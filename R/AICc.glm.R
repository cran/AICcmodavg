...
#check whether object is of appropriate class
  if(identical(class(mod), "lm") || identical(class(mod), c("glm", "lm"))) {
      ...
      if(c.hat > 1 && c.hat < 4) {K<-K+1; if(second.ord==TRUE) {AICc <- (-2*LL/c.hat)+2*K*(n/(n-K-1)) #adjust parameter count to include estimation of dispersion parameter
      ...
      if(!is.na(charmatch(x="Negative Binomial", table=family(mod)$family))) {if(is.null(mod$theta)) {K <- K+1;
