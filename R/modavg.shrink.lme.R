...
function(cand.set, parm, modnames, conf.level = 0.95, second.ord = TRUE, nobs = NULL,
  ...
  if(!identical(check.class, "lme"))  {stop("\nThis function is only appropriate with the \'lme\' class\n")}
  ...
  ##compute table
  new_table <- aictab.lme(cand.set = cand.set, modnames = modnames, sort = FALSE,
